...
public class Alarm {
	...
	public void timerInterrupt() {
		...
		for (int i = 0; i < alarmWaitQueue.size(); i++) {
			if (Machine.timer().getTime() >= alarmWaitQueue.get(i).wakeTime) {
				...
				System.out.println("唤醒线程：" + alarmWait.thread.getName() + "，时间为：" + Machine.timer().getTime());
	...
	public void waitUntil(long x) {
		...
		System.out.println(
				KThread.currentThread().getName() + "线程休眠，时间为：" + Machine.timer().getTime() + ",应在" + wakeTime + "醒来.");
		KThread.sleep();
	...
	private class AlarmWait {
		...
		/*
		 * 这些方法暂时用不上 public KThread getThread() { return thread; }
		 * 

...
public class PriorityScheduler extends Scheduler {
	...
	protected class PriorityQueue extends ThreadQueue {
		 ...
		 */
		private java.util.PriorityQueue<ThreadState> waitQueue = new java.util.PriorityQueue<ThreadState>(8,
				new ThreadStateComparator<ThreadState>(this));
		/**
		 * The <tt>KThread</tt> that locks this PriorityQueue. Initially it is
		 * null.
		 */
		...
		protected class ThreadStateComparator<T extends ThreadState> implements Comparator<T> {
			...
			public int compare(T o1, T o2) {
				...
				if (o1_effectivePriority > o2_effectivePriority) {
					...
					// compare by the times these threads have spent in this
					// queue
					long o1_waitTime = o1.waiting.get(priorityQueue), o2_waitTime = o2.waiting.get(priorityQueue);
					/*
					 * Note: the waiting maps should contain THIS at this point.
					 * If they don't then there is an error
					 */
	...
	protected class ThreadState {
		...
		ThreadState(KThread thread) {
			...
			setPriority(priorityDefault);// FIXME: Is this really necessary? At
											// this point it is not linked above
											// or below by anything.
		}

		/**
		 * Release this priority queue from the resources this ThreadState has
		 * locked.
		 * <p>
		 * This is the only time the effective priority of a thread can go down
		 * and needs a full recalculation.
		 * <p>
		 * We can detect if this exists if the top effective priority of the
		 * queue we are release is equal to this current effective priority. If
		 * it is less than (it cannot be greater by definition), then we know
		 * that something else is contributing to the effective priority of
		 * <tt>this</tt>.
		 * 
		 * @param priorityQueue
		 */
		 ...
		 * <p>
		 * This method assumes the priority has changed. Protection is from
		 * PriorityScheduler class calling this.
		 * 
		 * @param priority
		 *            the new priority.
		 */
		...
		void waitForAccess(PriorityQueue priorityQ) {
			if (!waiting.containsKey(priorityQ)) {
				...
				// The effective priority of this shouldn't change, so just
				// shove it onto the waitQueue's members
		...
		void acquire(PriorityQueue priorityQ) {
			...
			/*
			 * Remove the passed thread state from the queues, if it exists on
			 * them
			 */
		...
		/**
		 * A map of all the PriorityQueues this ThreadState is waiting on mapped
		 * to the time they were waiting on them
		 */
